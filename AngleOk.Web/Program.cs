...
//builder.Services.AddRazorPages();// добавляем в приложение сервисы Razor Pages
...
//builder.Services.AddMvcCore();//: добавляет только основные сервисы фреймворка MVC, а всю дополнительную функциональность, типа аутентификацией и авторизацией, валидацией и т.д., необходимо добавлять самостоятельно
builder.Services.AddControllersWithViews();//: добавляет только те сервисы фреймворка MVC, которые позволяют использовать контроллеры и представления и связанную функциональность. При создании проекта по типу ASP.NET Core Web App (Model-View-Controller) используется именно этот метод
...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        Version = "v1",
        Title = "AngleOk",
        Description = "Агенство недвижимости \"Уголок\". Описание методов API. Приложение разработано в рамках работы над курсовым проектом по дисциплине \"Базы данных\" ",
...
//app.MapRazorPages();// добавляем поддержку маршрутизации для Razor Pages
